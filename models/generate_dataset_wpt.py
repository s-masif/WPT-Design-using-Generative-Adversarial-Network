...
trace_widths = np.arange(0.06, 0.82, 0.04)  # Adjusted: Varying from 0.06 mm to 0.8 mm with a larger step
trace_thicknesses = np.arange(0.06, 0.82, 0.04)  # Adjusted: Varying from 0.06 mm to 0.8 mm with a larger step
gaps = np.arange(0.5, 2.5, 0.5)  # Adjusted: Gap between segments varies from 0.5 mm to 2.5 mm
center_area_percentages = np.arange(0.15, 0.30, 0.05)  # Adjusted: Varying center area percentage from 15% to 30%
...
def calculate_mutual_inductance_for_filaments(a, b, c, d, z):
    denominator = (a + c) * 2 + (b + d) * 2 + z ** 2
    if denominator == 0:
        return 0.0

    k_sq = 4 * a * c / denominator
    if k_sq < 0 or k_sq > 1:
        return 0.0
    ...
    sqrt_ac = np.sqrt(ac_product)
    M = (mu_0 / np.pi) * sqrt_ac * ((1 - k_sq / 2) * K_k - E_k)
...
def calculate_mutual_inductance_for_segment(segment_length_rx, segment_width_rx, segment_length_tx, segment_width_tx,
                                            N_rx, Tw_rx, T_rx, gap_length):
    if N_rx <= 0:
        return 0.0

    P_T_rx = max(int(Tw_rx / 0.02), 1)
    Q_T_rx = max(int(T_rx / 0.02), 1)
        ...
        for p_T_rx in range(1, 2 * P_T_rx + 2):
            for q_T_tx in range(1, 2 * Q_T_rx + 2):
                for p_T_tx in range(1, 2 * P_T_rx + 2):
                    ...
                    z = gap_length  # Separation between filaments
    ...
    normalization_factor = ((2 * Q_T_rx + 1) * (2 * P_T_rx + 1)) ** 2
    if normalization_factor == 0:
        return 0.0

    M_average = M_total / normalization_factor
    return M_average
        ...
        for T_rx in trace_thicknesses:
            for gap_length in gaps:
                ...
                dataset.append({
                    ...
                    'Number of Turns per Tx Segment': N_tx,
