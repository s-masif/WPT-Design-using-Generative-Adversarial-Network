...
trace_widths = torch.arange(0.06, 0.82, 0.04).to(device)  # Move to GPU
trace_thicknesses = torch.arange(0.06, 0.82, 0.04).to(device)  # Move to GPU
gaps = torch.arange(0.05, 1, 0.05).to(device)  # Move to GPU
center_area_percentages = torch.arange(0.15, 0.30, 0.05).to(device)  # Move to GPU

# Set constant distance between coils (z = 20 mm)
z = torch.tensor(20.0, device=device)  # Constant distance in mm
...
output_file = r'C:\Users\Andrew\PycharmProjects\pythonProject\Data\mutual_inductance_dataset_updated_9th_sept.csv'
...
def calculate_mutual_inductance_for_filaments(a, b, c, d, z):
    ...
    try:
        K_k = ellipk(k_sq.cpu().numpy())
        E_k = ellipe(k_sq.cpu().numpy())
    except ValueError:
        return torch.tensor(0.0, device=device)

    sqrt_ac = torch.sqrt(a * c)
    M = (mu_0 / math.pi) * sqrt_ac * ((1 - k_sq / 2) * K_k - E_k) / R

    if torch.isnan(M) or torch.isinf(M):
        return torch.tensor(0.0, device=device)
...
def calculate_mutual_inductance_for_segment(segment_length_rx, segment_width_rx, segment_length_tx, segment_width_tx,
    ...
    M_total = torch.tensor(0.0, device=device)
            ...
            for gap_length in gaps:
                ...
                if available_length_rx > 0 and available_length_tx > 0:
                    ...
                    if segment_length_rx_L > 0 and segment_length_rx_LC > 0 and segment_length_tx > 0 \
                        ...
                        dataset.append({
                            'Trace Width (Rx L mm)': Tw_rx.item(),
                            'Trace Thickness (Rx L mm)': T_rx.item(),
                            'Number of Turns (Rx L)': N_rx_L,
                            'Trace Width (Rx L+C mm)': Tw_rx.item(),
                            'Trace Thickness (Rx L+C mm)': T_rx.item(),
                            'Number of Turns (Rx L+C)': N_rx_LC,
                            'Gap Between Segments (mm)': gap_length.item(),
                            'Center Area Percentage': center_area_percentage.item(),
                            ...
                            'Mutual Inductance Segment L (H)': M1_L.item(),
                            'Mutual Inductance Segment L+C (H)': M1_LC.item(),
                            'Final Mutual Inductance (H)': M_final.item()
                        })
                        ...
                        if counter % 100 == 0 or counter == total_iterations:
                            ...
                            print(f"Processed {counter}/{total_iterations} configurations. Time for last 100 configurations: {elapsed_time:.2f} seconds.")
