...
# Constants
mu_0 = tf.constant(4 * np.pi * 1e-7, dtype=tf.float32)  # Permeability of free space (H/m)

# Define the updated Rx coil dimensions (in mm)
rx_total_width = 37.0  # Total available width for Rx segments
rx_total_length = 20.0  # Total available length for Rx segments
...
z = tf.constant(20.0, dtype=tf.float32)  # Constant distance in mm
...
def calculate_mutual_inductance_for_filaments(a, b, c, d, z):
    ...
    R = tf.sqrt((a + c) ** 2 + (b + d) ** 2 + z ** 2)
    k_sq = 4 * a * c / ((a + c) ** 2 + (b + d) ** 2 + z ** 2)

    # Use py_function to call scipy.special functions within the TensorFlow graph
    K_k = tf.cast(tf.py_function(func=ellipk, inp=[k_sq], Tout=tf.float32), dtype=tf.float32)
    E_k = tf.cast(tf.py_function(func=ellipe, inp=[k_sq], Tout=tf.float32), dtype=tf.float32)

    sqrt_ac = tf.sqrt(a * c)
    M = (mu_0 / tf.constant(np.pi, dtype=tf.float32)) * sqrt_ac * ((1 - k_sq / 2) * K_k - E_k) / R

    # Handle any NaNs or Infs
    M = tf.where(tf.math.is_nan(M), tf.constant(0.0, dtype=tf.float32), M)
    M = tf.where(tf.math.is_inf(M), tf.constant(0.0, dtype=tf.float32), M)
...
def calculate_mutual_inductance_for_segment(segment_length_rx, segment_width_rx, segment_length_tx, segment_width_tx,
    ...
    M_total = tf.constant(0.0, dtype=tf.float32)
...
with tf.device('/GPU:0'):  # Run the entire loop on GPU
                    ...
                    if available_length_rx > 0 and available_length_tx > 0:
                        ...
                        if segment_length_rx_L > 0 and segment_length_rx_LC > 0 and segment_length_tx > 0 \
                            ...
                            dataset.append({
                                ...
                                'Mutual Inductance Segment L (H)': M1_L.numpy(),
                                'Mutual Inductance Segment L+C (H)': M1_LC.numpy(),
                                'Final Mutual Inductance (H)': M_final.numpy()
                            })
